        ...
        <div className="relative">
          ...
          <div className="space-y-20">
            {TIMELINE.map((item, index) => {
                ...
                return (
                    <div key={item.id} className={`relative flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} gap-8 md:gap-0 group`}>
                        {/* Timeline Dot */}
                        ...
                        <div className={`md:w-1/2 ${isEven ? 'md:pr-12 md:text-right' : 'md:pl-12'}`}>
                            <div className={`flex flex-col ${isEven ? 'md:items-end' : 'md:items-start'}`}>
                                <div className="flex items-center gap-3 mb-2">
                                     {/* Company Logo - Attempting to display logo, fallback to text if broken */}
                                     {/* Note: In a real scenario, we'd use the uploaded blobs. Here we simulate the look. */}
                                    {!isEven && item.logo && (
                        ...
                        {/* Spacer/Detail Side (for alternating layout) */}
                        <div className={`md:w-1/2 ${isEven ? 'md:pl-12' : 'md:pr-12 md:text-right'}`}>
                             {!isEven && (
                                <ul className="space-y-3 text-gray-600 dark:text-gray-400 list-disc pl-5 md:list-none">
                                    {item.description.map((desc, i) => (
                                        <li key={i}>{desc}</li>
                                    ))}
                                </ul>
